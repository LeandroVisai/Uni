...
library(stargazer) # Para las tablas de resultados
...
# 1. Establece tu directorio de trabajo en la carpeta de la Tarea 2
setwd("/Users/leandrovenegas/Documents/GitHub/Uni/Ingenieria de marketing/Tarea_2")

# 2. Ahora, lee el archivo (R lo encontrará en esa carpeta)
...
# --- 4. Preparar Datos para mlogit (¡CON CORRECCIÓN!) ---

# 4.1. PASO NUEVO: Limpiar y forzar tipos de datos
# (Las columnas Znit y Ref_Price_Znit ahora existirán, así que esto funcionará)
...
# 4.2. Formatear Datos para mlogit
...
# --- 5. Estimación de los 3 Modelos Logit (P2.a) ---

# (Añadimos 'na.action = na.omit' por seguridad)

# Modelo 1 (Base):
m1_logit <- mlogit(CHOICE ~ PRICE + QUAL + FASH | 1, 
                   data = data.mlogit, na.action = na.omit)

# Modelo 2 (Base + Lealtad Znit):
m2_logit <- mlogit(CHOICE ~ PRICE + QUAL + FASH + Znit | 1, 
                   data = data.mlogit, na.action = na.omit)

# Modelo 3 (Base + Lealtad Znit + P. Referencia Znit DIRECTO):
m3_logit_directo <- mlogit(CHOICE ~ PRICE + QUAL + FASH + Znit + Ref_Price_Znit | 1, 
                           data = data.mlogit, na.action = na.omit)

# --- 6. Ver Resultados (P2.a) ---
# (Esto ahora debería funcionar, ya que m1, m2 y m3 se crearán correctamente)
stargazer(m1_logit, m2_logit, m3_logit_directo, type = "text",
