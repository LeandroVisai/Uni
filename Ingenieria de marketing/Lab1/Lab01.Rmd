...
Para programar en _R_, se puede hacer directamente sobre un archivo plano (.R). Este es un archivo R Markdown (.Rmd) que te permite integrar código, con los resultados y además comentarios en texto para poder ordenar y discutir el análisis que esten haciendo. Las salidas se pueden exportar a formato HTML, PDF y MS Word, lo que facilita la comunicación con los equipos de trabajo. Este formato no solo se adecua bien para que ustedes comuniquen sus avances, sino que también es bastante usado en equipos de desarollo.

En esencia, un archivo markdown es un documento de texto estructurado en el que podemos incluir títulos, subtitulo y listas, en el que podemos incrustar bloques de código R como este:
...
Si quieren ponerse creativos, también pueden incluir enlaces. Si quieres agregar un enlace, debes poner entre llaves cuadradas el string que acompañará al hipervínculo y entre llaves redondas el enlace.
...
+ Para ver el output de tu archivo markdown presiona ctrl+shift+k (o clickea en _Knit_)
...
Como en casi todo lenguaje, podemos definir variables y asignarles valor. Los objetos quedan en el espacio de trabajo y podemos manipularlos a nuestra disposición. Por ejemplo los podemos imprimir.  
```{r}
var1 <- 5+1
print(var1)
```
...
R también aguanta la definición de funciones. Cuando queramos hacer algo muchas veces es mejor tener una función general que lo haga que escribir la secuencia de comandos multiples veces. 
...
R funciona con multiples tipos, pero con frecuencia vamos a usar dataframes. Los dataframes son basicamente una tabla que permite multiples tipos de datos. 
```{r}
set.seed(27)
y = 11:20
...
head(mi_df, 4) # imprime las primeras 4 filas del dataframe
```

Hay múltiples operaciones que podemos hacer sobre los dataframes. Por ejemplo agregar una columna, selecccionar alguna casilla especifica, una columna completa o generar subconjuntos. 
```{r}
#agregamos una columna z
mi_df$z<-3:12
head(mi_df, 4)
#seleccionamos una casilla
print(mi_df[2,1])
#Devuelve una lista
print(mi_df["x"])
#devuelve un vector
