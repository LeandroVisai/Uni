...
```{r}
# 2. Pasar a formato largo (de columnas de países a columna Pais + TipoCambio)
...
datos_precios$Ano <- as.integer(datos_precios$Ano)

str(datos_precios)


# 3. Unir con la base de precios (df_precios)
...
datos_precios <- datos_precios %>%
  mutate(Precio_Unitario = Precio_Final / Peso_Volumen)
...
precio_por_cadena <- datos_precios %>% group_by(Pais,Ano,Mes, Cadena, Sub_categoria) %>%
  summarise(Precio_Unitario_Promedio = mean(Precio_Unitario, na.rm = TRUE)) #%>%
  #ungroup() # Es una buena práctica desagrupar al final
View(precio_por_cadena)
```

```{r agrupamos por sub_categoria Y Pais}
precio_por_pais <- datos_precios %>%
  group_by(Pais,Ano,Mes, Sub_categoria) %>%
  summarise(Precio_Unitario_Promedio = mean(Precio_Unitario, na.rm = TRUE))# %>%
  #ungroup() # Es una buena práctica desagrupar al final
...
# Ver valores únicos
unique(datos_precios$Pais)   # lista de países
unique(datos_precios$Ano)    # lista de años
unique(datos_precios$Mes)    # lista de meses
