...
# --- (Opcional) Graficar para una comparación visual ---
...
costo_observado_ajustado <- data_ag %>%
  # La condición ahora incluye el Mes 2 y el Mes 3
  filter(Ano == 2022, Mes %in% c(2, 3)) %>%
  right_join(definicion_canastas, by = "Sub_categoria") %>%
  group_by(Pais, Canasta) %>%
  # Usamos mean() para promediar los precios de los dos meses
  summarise(Costo_Observado_USD = sum(Precio_Unitario_Promedio, na.rm = TRUE), .groups = 'drop')
