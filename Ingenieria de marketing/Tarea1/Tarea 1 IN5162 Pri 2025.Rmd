...
```{r eliminamos subcategorías que están mal}
# Eliminar subcategorías específicas
precio_por_pais <- precio_por_pais %>%
...
```{r borramos las categorías que tienen precios malos para visualizar bien}
...
```{r}
# 1. Generar base con precios por producto (subcategoría)
precio_por_pais <- datos_precios_tc %>%
  group_by(Ano, Sub_categoria, Pais) %>%
  summarise(Precio_Unitario_Promedio = mean(PrecioU_oferta_usd, na.rm = TRUE)) %>%
  ungroup()

# 2. Ranking por producto y año
ranking_productos <- precio_por_pais %>%
  group_by(Ano, Sub_categoria) %>%
  mutate(Rank_del_Pais = rank(Precio_Unitario_Promedio, ties.method = "first")) %>%
  ungroup()

# 3. Ranking agregado por país
...
# Cargarmos la base macro.
macro <- read.csv("variables_macro.csv")

# Unir bases por País y Año, con variables macros para pregunta 2.
data_completa <- precio_por_pais %>%
...
# Base de datos: precios por país-subcategoría-año
macro <- read.csv("variables_macro.csv")
...
# ================================
# 1. Especificación con variables macro
# ================================
macro <- read.csv("variables_macro.csv")

# Unimos precios país-año con variables macro
data_macro <- precio_por_pais %>%
  inner_join(macro, by = c("Pais", "Ano")) %>%
  mutate(GDPpc_miles = GDPpc / 1000)  # Escalamos PIB per cápita

# Modelo 1: determinantes macroeconómicos
modelo_macro <- lm(
  Precio_Promedio_Pais ~ GDPpc_miles + Arancel + UrbGrow + 
    LPI + Inflacion + Import_PIB + FoodImp,
  data = data_macro
)

summary(modelo_macro)

#De acá se puede ver que hace sentido pues a mayor arancel da mayor precio y de forma significativa
#Así mismo con GDP


# ================================
# 2. Especificación con variables internas (categorías comunes)
# ================================

# 1. Construimos la base filtrando categorías comunes
precio_por_categoria <- datos_precios_tc %>%
...
# 2. Pasamos a formato ancho
precio_categorias_comunes_wide <- precio_categorias_comunes %>%
  pivot_wider(names_from = Categoria, values_from = Precio_Unitario_Promedio)

# 3. Unimos con la base país-año
data_categorias <- precio_por_pais %>%
  inner_join(precio_categorias_comunes_wide, by = c("Pais", "Ano"))

# 4. Seleccionamos un subconjunto de categorías representativas
vars_categorias <- c("Aguas",
                     "Bebidas alcohólicas",
                     "Frutas y vegetales",
                     "Carne, aves y sucedáneos de la carne",
                     "Productos lácteos y sucedáneos lácteos",
                     "Granos, pasta y masas")

# 5. Construimos la fórmula asegurando backticks
fmla <- as.formula(
  paste("Precio_Promedio_Pais ~",
        paste(sprintf("`%s`", vars_categorias), collapse = " + "))
)

# 6. Modelo con variables internas (solo esas categorías)
modelo_categorias <- lm(fmla, data = data_categorias)
...
modelsummary(
  list(
    "Macro (PIB, Aranceles...)" = modelo_macro,
    "Categorías internas" = modelo_categorias
  ),
  stars = TRUE,
  output = "markdown"
)
...
#### Pregunta 2
# Usando los aprendizajes derivados de la exploración de datos, 
# utilicen un enfoque de regresión lineal para examinar cuantitativamente 
# qué factores determinan los distintos niveles de precios que pueden haber.

# ================================
# 1. Especificación con variables macro
# ================================

# Base con unión de precios promedio y variables macro
macro <- read.csv("variables_macro.csv")

data_completa <- precio_por_pais %>%
  inner_join(macro, by = c("Pais", "Ano"))

# Modelo solo con determinantes macroeconómicos
modelo_macro <- lm(
  Precio_Unitario_Promedio ~ GDPpc + Arancel + UrbGrow + 
    LPI + Inflacion + Import_PIB + FoodImp,
  data = data_completa
)
...
# ================================
# 2. Especificación con variables micro
# ================================

# Generamos una base más desagregada a nivel producto/cadena
# Nota: ajusta la detección de "marca propia" según cómo esté en tu base
library(stringr)

data_micro <- datos_precios_tc %>%
  mutate(MarcaPropia = ifelse(str_detect(Sub_categoria, "marca propia"), 1, 0)) %>%
  select(Pais, Ano, Categoria, Sub_categoria, Cadena, MarcaPropia, PrecioU_oferta_usd)

# Modelo con variables microeconómicas internas de la base
modelo_micro <- lm(
  PrecioU_oferta_usd ~ MarcaPropia + Categoria + Cadena + Pais,
  data = data_micro
)

summary(modelo_micro)


# ================================
# 3. Comparación de modelos lado a lado
# ================================
#library(modelsummary)
...
#modelsummary(
 # list(
    "Macro (PIB, Aranceles...)" = modelo_macro,
    "Micro (Categoría, Marca, Cadena)" = modelo_micro
 # ),
#  stars = TRUE,      # Agregar significancia con asteriscos
 # output = "markdown" # Puede ser "latex" si lo exportas a informe
#)
```
...
```{r P2_b}
# ================================
# Selección automática de variables con LASSO (sin País como regresor)
# ================================
library(glmnet)
library(dplyr)

## ---- Modelo Macro ----
data_macro_lasso <- data_completa %>%
  select(Precio_Unitario_Promedio, GDPpc, Arancel, UrbGrow, 
         LPI, Inflacion, Import_PIB, FoodImp) %>%
  na.omit()

y_macro <- data_macro_lasso$Precio_Unitario_Promedio
X_macro <- model.matrix(Precio_Unitario_Promedio ~ ., data = data_macro_lasso)[, -1]

cv_macro <- cv.glmnet(X_macro, y_macro, alpha = 1, standardize = TRUE)

cat("Lambda óptimo modelo macro:", cv_macro$lambda.min, "\n")

coef_macro <- coef(cv_macro, s = "lambda.min")
cat("Variables seleccionadas (macro):\n")
print(rownames(coef_macro)[which(coef_macro != 0)])


## ---- Modelo Micro (sin País) ----
data_micro_lasso <- data_micro %>%
  select(PrecioU_oferta_usd, MarcaPropia, Categoria, Cadena) %>%  # <- quitamos Pais
  na.omit()

y_micro <- data_micro_lasso$PrecioU_oferta_usd
X_micro <- model.matrix(PrecioU_oferta_usd ~ ., data = data_micro_lasso)[, -1]

cv_micro <- cv.glmnet(X_micro, y_micro, alpha = 1, standardize = TRUE)

cat("Lambda óptimo modelo micro:", cv_micro$lambda.min, "\n")

coef_micro <- coef(cv_micro, s = "lambda.min")
cat("Variables seleccionadas (micro):\n")
print(rownames(coef_micro)[which(coef_micro != 0)])

```

```{r P2_b otra forma}
# ================================
# Selección automática con Stepwise + Ridge
# ================================
library(glmnet)
library(MASS)   # <- AQUÍ está stepAIC()

## ---- Modelo Macro ----
# 1. Preparamos datos
data_macro_ridge <- data_completa %>%
  dplyr::select(Precio_Unitario_Promedio, GDPpc, Arancel, UrbGrow, 
                LPI, Inflacion, Import_PIB, FoodImp) %>%
  na.omit()

# 2. Ridge base con glmnet
y_macro <- data_macro_ridge$Precio_Unitario_Promedio
X_macro <- model.matrix(Precio_Unitario_Promedio ~ ., data = data_macro_ridge)[, -1]

cv_ridge_macro <- cv.glmnet(X_macro, y_macro, alpha = 0, standardize = TRUE)
cat("Lambda óptimo modelo macro (Ridge):", cv_ridge_macro$lambda.min, "\n")

# 3. Stepwise (AIC) sobre modelo OLS
modelo_macro_full <- lm(Precio_Unitario_Promedio ~ GDPpc + Arancel + UrbGrow +
                          LPI + Inflacion + Import_PIB + FoodImp,
                        data = data_macro_ridge)

step_macro <- stepAIC(modelo_macro_full, direction = "both", trace = FALSE)

summary(step_macro)
cat("Variables seleccionadas en modelo macro (Stepwise):\n")
print(names(coef(step_macro))[-1])  # quitamos intercepto


## ---- Modelo Micro ----
# 1. Preparamos datos
data_micro_ridge <- data_micro %>%
  dplyr::select(PrecioU_oferta_usd, MarcaPropia, Categoria, Cadena) %>%
  na.omit()

# 2. Ridge base
y_micro <- data_micro_ridge$PrecioU_oferta_usd
X_micro <- model.matrix(PrecioU_oferta_usd ~ ., data = data_micro_ridge)[, -1]

cv_ridge_micro <- cv.glmnet(X_micro, y_micro, alpha = 0, standardize = TRUE)
cat("Lambda óptimo modelo micro (Ridge):", cv_ridge_micro$lambda.min, "\n")

# 3. Stepwise (AIC) sobre modelo OLS
modelo_micro_full <- lm(PrecioU_oferta_usd ~ MarcaPropia + Categoria + Cadena,
                        data = data_micro_ridge)

step_micro <- stepAIC(modelo_micro_full, direction = "both", trace = FALSE)

summary(step_micro)
cat("Variables seleccionadas en modelo micro (Stepwise):\n")
print(names(coef(step_micro))[-1])
...
# Especificación 1: Precio_Unitario_Promedio ~ Import_PIB + Arancel
model1 <- lm(Precio_Unitario_Promedio ~ GDPpc + Arancel, data = data_completa)
summary(model1)

# Especificación 2: Precio_Unitario_Promedio ~ UrbGrow + LPI
model2 <- lm(Precio_Unitario_Promedio ~ UrbGrow + LPI, data = data_completa)
summary(model2)
