...
Luego de ver visualmente la distribución de variables, podriamos estudiar el ranking de paises baratos/caros, usando un posible ranking.

```{r}


# 1. Para cada producto (Sub_categoria y Ano), rankeamos los países del más barato (Rank=1) al más caro.
#    Quitamos el argumento 'ascending = TRUE' porque no existe en la función rank().
...
# 2. Ahora, sumamos los rankings que cada país obtuvo en los diferentes productos.
#    El país con la suma total más baja es, en promedio, el más barato.
ranking_final <- ranking_productos %>%
  group_by(Pais) %>%
  summarise(RankTotal = sum(Rank_del_Pais, na.rm = TRUE)) %>%
  arrange(RankTotal) # Ordenamos del más barato al más caro
