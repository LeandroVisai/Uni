...
# Crear el dataframe con los datos de PPA que obtuviste
# (Tal como los tienes, en formato ancho)
ppa_ancho <- data.frame(
  ...
  check.names = FALSE # Para que R acepte los nombres de columna numéricos "2021" y "2022"
)

# Convertir el dataframe de PPA a formato "largo" 
# Esto crea una fila por cada país y año, que es lo que necesitamos
ppa <- ppa_ancho %>%
  pivot_longer(
  ...
  mutate(
    Ano = as.integer(Ano) # Convertimos la columna 'Ano' a número
    )

#Unir los datos de PPA con tu base de datos macro 
data_macro_con_ppa <- data_macro %>%
  left_join(ppa, by = c("Pais", "Ano"))


# Calcular el precio corregido por PPA
data_macro_con_ppa <- data_macro_con_ppa %>%
  mutate(Precio_PPA = Precio_Unitario_Promedio / Factor_PPA)
...
# (opcional pero útil) fuerza a que select sea de dplyr
select <- dplyr::select
...
# Asumimos que la lista de modelos 'model_list' (con modelo_rf) ya existe
# y fue entrenada con el script anterior.

# --- 1. Definir las canastas de abarrotes ---
...
# Creamos un dataframe con la definición de las canastas para usarlo después
definicion_canastas <- data.frame(
  ...
  Canasta = rep(c("Básica", "Carnes y Verduras", "Aseo del Hogar"), 
                times = c(length(canasta_basica), length(canasta_carnes_verduras), length(canasta_aseo_hogar)))
)

# --- 2. Crear el escenario para la predicción ---
# Vamos a predecir los precios para Marzo de 2022 en todos los países.
escenario_prediccion <- expand.grid(
  Sub_categoria = definicion_canastas$Sub_categoria,
  Pais = levels(train_df$Pais), # Usamos los niveles del set de entrenamiento
  Ano = 2022,
  Mes = 3) %>%
  distinct()

# --- 3. Pronosticar precios usando el mejor modelo (Random Forest) ---
escenario_prediccion$Precio_Predicho_USD <- predict(model_list$RF, newdata = escenario_prediccion)

# --- 4. Calcular el costo total de cada canasta por país ---
...
# --- 5. Presentar resultados y comparar ---
...
```{r}
# --- 1. Calcular el costo REAL OBSERVADO de cada canasta (promedio de Feb y Mar 2022) ---
costo_observado_ajustado <- data_ag %>%
  ...
  summarise(Precio_Promedio_Obs = mean(Precio_Unitario_Promedio, na.rm = TRUE), .groups = 'drop') %>%
...
# --- 2. Unir la tabla de pronósticos con la de costos observados ajustados ---
...
# --- 3. Presentar la tabla comparativa final ---
...
Tendremos que convertir la base con respecto a ppa, ya que ya corregimos por moneda.



 Yo sacaria lo en verde es como exotico, carga de librerias algo que ya se hizo.

```{r P4b2}

# Chequeos mínimos
stopifnot(exists("precio_por_subcategoria"), exists("Tipo_Cambio"), exists("ppa"), exists("definicion_canastas"))

#suppressPackageStartupMessages({
#  library(dplyr); library(caret); library(ggplot2); library(knitr)
#})
#if ("package:plyr" %in% search()) detach("package:plyr", unload=TRUE)
#select <- dplyr::select
set.seed(5162)

# 1) Construir precios en dólares internacionales (Int$) -------------------------
# Definiciones:
#   Precio_Int = Precio_USD / ( TipoCambio(USD/LCU) * Factor_PPA(LCU/Int$) )
# Nota: si tu Precio_Unitario_Promedio estuviera en moneda local, sería: Precio_local / Factor_PPA
base_int <- precio_por_subcategoria %>%
  dplyr::left_join(Tipo_Cambio, by = c("Pais","Ano","Mes")) %>%   # aporta TipoCambio (USD/LCU)
  dplyr::left_join(ppa,         by = c("Pais","Ano")) %>%          # aporta Factor_PPA (LCU/Int$)
  dplyr::mutate(
    Divisor_Int = TipoCambio * Factor_PPA,
    Precio_Int  = Precio_Unitario_Promedio / Divisor_Int
  ) %>%
  dplyr::filter(is.finite(Precio_Int))

# 2) Sanity checks útiles --------------------------------------------------------
# a) pérdidas por joins
faltan_tc  <- dplyr::anti_join(precio_por_subcategoria, Tipo_Cambio, by=c("Pais","Ano","Mes"))
faltan_ppa <- dplyr::anti_join(precio_por_subcategoria, ppa,         by=c("Pais","Ano"))
if (nrow(faltan_tc)  > 0) warning("Filas sin Tipo_Cambio: ", nrow(faltan_tc))
if (nrow(faltan_ppa) > 0) warning("Filas sin Factor_PPA: ", nrow(faltan_ppa))

# b) ratio mediano Int$/USD por país (debería ser ~1–2; no 0.01)
auditoria_ratio <- base_int %>%
  dplyr::group_by(Pais) %>%
  dplyr::summarise(
    med_USD = median(Precio_Unitario_Promedio, na.rm=TRUE),
    med_Int = median(Precio_Int,               na.rm=TRUE),
    ratio   = med_Int/med_USD
  ) %>% dplyr::arrange(ratio)
print(auditoria_ratio)

# 3) Dataset de modelado en Int$ -------------------------------------------------
data_ag_int <- base_int %>%
  dplyr::select(Ano, Mes, Pais, Sub_categoria, Precio_Int) %>%
  dplyr::mutate(dplyr::across(c(Pais, Sub_categoria), as.factor))

idx  <- caret::createDataPartition(data_ag_int$Sub_categoria, p = 0.8, list = FALSE)
train <- data_ag_int[idx, ]
test  <- data_ag_int[-idx, ]
fml   <- Precio_Int ~ Ano + Mes + Pais + Sub_categoria   # (~ . también funciona)

# 4) Entrenamiento (LM / MARS / RF) ---------------------------------------------
ctrl <- caret::trainControl(method="cv", number=10, savePredictions="final")

modelo_lm_int <- caret::train(fml, data=train, method="lm", trControl=ctrl)

modelo_mars_int <- caret::train(
  fml, data=train, method="earth", trControl=ctrl,
  tuneGrid=expand.grid(degree=1, nprune=5:12)
)

modelo_rf_int <- caret::train(
  fml, data=train, method="rf", trControl=ctrl,
  tuneGrid=expand.grid(mtry=2:4), ntree=100
)

model_list_int <- list(LM=modelo_lm_int, MARS=modelo_mars_int, RF=modelo_rf_int)

# 5) Escenario: predicción para Mar/2022 en canastas definidas -------------------
escenario_pred <- expand.grid(
  Sub_categoria = definicion_canastas$Sub_categoria,
...
escenario_pred$Precio_Predicho_Int <- predict(model_list_int$RF, newdata = escenario_pred)

# 6) Costo por canasta (PREDICHO en Int$) ---------------------------------------
costo_pred_canastas <- escenario_pred %>%
  dplyr::left_join(definicion_canastas, by="Sub_categoria") %>%
  dplyr::group_by(Pais, Canasta) %>%
  dplyr::summarise(Costo_Pronosticado_Int = sum(Precio_Predicho_Int), .groups="drop")

# 7) Costo OBSERVADO (promedio Feb–Mar 2022) en Int$ ----------------------------
costo_obs_canastas <- base_int %>%
  dplyr::filter(Ano==2022, Mes %in% c(2,3)) %>%
  dplyr::semi_join(definicion_canastas, by="Sub_categoria") %>%
  dplyr::group_by(Pais, Sub_categoria) %>%
  dplyr::summarise(Precio_Promedio_Int = mean(Precio_Int, na.rm=TRUE), .groups="drop") %>%
  dplyr::left_join(definicion_canastas, by="Sub_categoria") %>%
  dplyr::group_by(Pais, Canasta) %>%
  dplyr::summarise(Costo_Observado_Int = sum(Precio_Promedio_Int, na.rm=TRUE), .groups="drop")

# 8) Tabla comparativa Predicho vs Observado (Int$) ------------------------------
tabla_comparativa_int <- costo_pred_canastas %>%
  dplyr::left_join(costo_obs_canastas, by=c("Pais","Canasta"))

knitr::kable(tabla_comparativa_int,
             caption = "Costo de Canastas en dólares internacionales (Int$): Pronosticado vs Observado",
             digits = 2)

# 9) Comparación respecto a Chile (Int$) ----------------------------------------
comp_vs_chile <- costo_pred_canastas %>%
  dplyr::group_by(Canasta) %>%
  dplyr::mutate(
    Costo_Chile    = Costo_Pronosticado_Int[ Pais=="Chile" ],
    Delta_vs_Chile = Costo_Pronosticado_Int - Costo_Chile,
    Ratio_vs_Chile = Costo_Pronosticado_Int / Costo_Chile
  ) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(Canasta, Ratio_vs_Chile)
print(comp_vs_chile)

# 10) (Opcional) gráfico rápido Int$ por canasta --------------------------------
ggplot(costo_pred_canastas, aes(x=reorder(Pais, Costo_Pronosticado_Int),
                                y=Costo_Pronosticado_Int, fill=Pais)) +
  geom_bar(stat="identity") +
  facet_wrap(~ Canasta, scales="free_x") +
  coord_flip() +
  labs(title="Costo Pronosticado de Canastas por País (Mar 2022) en Int$",
       x="País", y="Costo Total (Int$)") +
  theme_minimal() +
  theme(legend.position="none")

# Tabla comparativa final: Pronóstico vs Observado (en Int$)
tabla_final <- tabla_comparativa_int %>%
  dplyr::mutate(
    Diferencia = Costo_Pronosticado_Int - Costo_Observado_Int,
    Ratio      = Costo_Pronosticado_Int / Costo_Observado_Int
  )

knitr::kable(
  tabla_final,
  caption = "Comparación de Costos de Canastas: Pronosticado vs Observado (en dólares internacionales)",
  digits = 2
)
